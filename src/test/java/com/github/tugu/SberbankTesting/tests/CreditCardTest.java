...
public class CreditCardTest {
	...
	public void testCreditCard() throws BiffException, IOException, InterruptedException {		
		...
		Assert.assertEquals(startPageTabsOnWebsite, expectedStartPageTabs, "Закладки на главной странице не совпадают");
		...
		// Переходим на закладку Выбрать карту  > кредитные карты  > универсальные кредитные карты  > премиальные карты
		Actions actions = new Actions(driver);
		actions.moveToElement(driver.findElementByXPath("//a[text()[contains(.,'Выбрать')] and text()[contains(.,'карту')]]"));		
		actions.perform();
		...
		for (int i = 0; i < creditCardSheet.getRows(); i++) {
			WebElement infoElement = driver.findElementByXPath(String.format("//table[@class='product-specific']//tr[%d]//td[1]", i+1));
			WebElement valueElement = driver.findElementByXPath(String.format("//table[@class='product-specific']//tr[%d]//td[2]", i+1));
			String infoExpected = creditCardSheet.getCell(0, i).getContents();
			String valueExpected = creditCardSheet.getCell(1, i).getContents();
			Assert.assertEquals(infoElement.getText(), infoExpected, "Название параметра карты не совпадает");
			Assert.assertEquals(valueElement.getText(), valueExpected, String.format("Значение параметра '%s' карты не совпадает", infoExpected));			
		}		
	...
	private ArrayList<String> getCreditLinksFromStartPage() {
		...
		ArrayList<WebElement> links = new ArrayList<WebElement>(driver.findElementsByXPath("//b[text()='Кредиты']/../../../../div[1]//ul//a"));
